...
while np.sum(clusters) < Nstr:
   ...
   sampledist = np.random.exponential(S,size=int(1e6))
   members = np.floor(size/C)#np.random.poisson(size/C)
   trueclusters = np.append(trueclusters,members)
   clusters = np.append(clusters,np.floor(members*sampledist))
   it+=1
